...
1. 这个分支主要是自用。程序支持 mlx 硬件加速和并行运算，可以一次传入多个视频文件。记得调整性能限制。
2. '--similarity'参数越大，筛出来的幻灯片越多，反之亦然。
3. 程序支持 DEBUG 模式。此模式下，缓存文件在本次任务结束后不再被删除，便于对长录播视频调整参数（免得每一次都要跑完整个视频）。另一种方法是使用'--start_frame'和'--end_frame'参数指定需要处理的时间范围，但是我发现总体来说不是很好用，因为切幻灯片的时候搞不好在哪一页突然有一个带文字的元素一闪而过。
4. 程序支持'--interval'参数，用于指定抽帧间隔，默认值为 1 秒。用于解决存在于抽帧间隔中的元素一闪而过而被漏掉的问题。
5. 程序支持指定差异识别算法。具体见下。
...
python -m video2ppt.video2ppt --similarity 0.96 --metric ssim '视频文件路径（支持单个或多个）或者视频文件所在目录' --interval 0.5 --debug
